...
class PersonelMenuDetayPage extends StatefulWidget {
  const PersonelMenuDetayPage({Key? key}) : super(key: key);

  @override
  State<PersonelMenuDetayPage> createState() => _PersonelMenuDetayPageState();
}

class _PersonelMenuDetayPageState extends State<PersonelMenuDetayPage> {
  @override
  void initState() {
        ...
        child: ElevatedButton(
          ...
          onPressed: () {
            if (_baslik.text.length < 2) {
              ...
              debugPrint('1' + _urun.ucret.toString());

              for (var i = 0; i < _cafe.menu!.kategori!.length; i++) {
                if (_cafe.menu!.kategori![i].name == _kategoriname) {
                  _cafe.menu!.kategori![i].urun![_urunind] = _urun;
                  debugPrint('2' +
                      _cafe.menu!.kategori![i].urun![_urunind].ucret
                          .toString());
                }
              }
